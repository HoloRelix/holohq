...
export default async function handler(req) {
  ...
  try {
    ...
    else if (condition === "Raw NM" || condition === "NM") q += " near mint";
    else if (condition === "Raw LP" || condition === "LP") q += " lightly played";
    if (set && set.length < 25) q += ` ${set}`;

    // Use Browse API with completed/sold filter
    const url = new URL("https://api.ebay.com/buy/browse/v1/item_summary/search");
    url.searchParams.set("q", q);
    url.searchParams.set("limit", "20");
    url.searchParams.set("sort", "-endDate");
    url.searchParams.set("filter", "buyingOptions:{FIXED_PRICE},conditions:{USED|LIKE_NEW}");
    ...
    if (data.errors) throw new Error(data.errors[0]?.longMessage || "eBay API error");
    ...
    const filtered = median > 0 ? sorted.filter(i => i.price >= median * 0.2 && i.price <= median * 4) : sorted;
